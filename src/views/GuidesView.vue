...
<template>
  <section>
    <header class="mb-6">
      ...
      <p class="text-sm text-gray-500">Save links to external resources.</p>
    </header>
    ...
    <form @submit.prevent="onAdd" class="mb-6 flex flex-wrap gap-2">
      ...
      <input v-model="url" placeholder="example.com or https://..." required
    ...
    <div v-else class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      <div v-for="g in store.guides" :key="g.id"
        ...
        <a :href="g.url" target="_blank" rel="noopener noreferrer" class="block pr-6">
          <div class="font-medium truncate">{{ g.title }}</div>
          <div class="mt-1 text-xs text-gray-500 truncate">
            {{ displayHost(g.url) }}
          </div>
        </a>
        <button @click="store.remove(g.id)" title="Remove"
